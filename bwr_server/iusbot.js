...
bot.on('ready', () => {
	...
	bot.channels.cache.get('856136777764962344').send('Hello here from BWR! [ius]')
});
const io = require("socket.io-client");
var socket = io("ius.bonkeyword.tk");
				...
				if (data.name != "BonziWORLD Revived Bot (s!)") {
					if (!data.name.includes("@everyone") && !data.includes("@here")) {
						if ( !text.includes("@everyone") && !text.includes("@here")) {
							bot.channels.cache.get('856136777764962344').send('**' + this.public.name + ' said: **`' + text + '`');
						}
					}
				} 
				if(text.startsWith('s!')){
