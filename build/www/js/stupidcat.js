// Go play the raymond version if you want to see the original. I give credits. ~ ICS
...
function checkforfunnysecret(slapcount) {
	...
	if (slapcount >= 110000) {
		amusingsecretdiv.innerHTML = `AAAAAAAAAAAAAAA`;
	}
	else if (slapcount >= 101000) {
		amusingsecretdiv.innerHTML = `grumm cute`;
	}
	else if (slapcount >= 100000) {
		amusingsecretdiv.innerHTML = `Losky has been sent to hell, this is merely a figment if your imagination at this point. Will you continue?`;
	}
		...
		amusingsecretdiv.innerHTML = `slap losky`;
	}
	else if (slapcount === 201) {
		amusingsecretdiv.innerHTML = `haha just kidding`;
	}
	else if (slapcount === 200) {
		amusingsecretdiv.innerHTML = `losky is dead now you may stop`;
		var unfunnycat = document.getElementById("stupidcat");
		unfunnycat.src = "./img/slaplosky_hurt.png";
		setTimeout(() => { unfunnycat.src = "./img/losky_hurt.png"; }, 100);
	}
	else if (slapcount >= 100) {
		amusingsecretdiv.innerHTML = `wow, you really do hate losky. good.`;
	}
...
function funnyslap() {
	if (localStorage["slapcount"] >= 200) {
		var unfunnycat = document.getElementById("stupidcat");
		unfunnycat.src = "./img/slaplosky_hurt.png";
		setTimeout(() => { unfunnycat.src = "./img/losky_hurt.png"; }, 100);
	} else if (localStorage["slapcount"] >= 4000) {
		var unfunnycat = document.getElementById("stupidcat");
		unfunnycat.src = "./img/slaplosky_hurt2.png";
		setTimeout(() => { unfunnycat.src = "./img/losky_hurt2.png"; }, 100);
	} else if (localStorage["slapcount"] <= 200) {
		var unfunnycat = document.getElementById("stupidcat");
		unfunnycat.src = "./img/slaplosky.png";
		setTimeout(() => { unfunnycat.src = "./img/losky.png"; }, 100);
	}
}
